...
class Samtools:
    ...
    def flagstat(self, bam):
        ...
        feat_map    = { 
            re.compile("(\d+)\s\+\s\d+\sin total \(QC-passed reads \+ QC-failed reads\)"):tuple(["qc_fail"]),
            re.compile("(\d+)\s\+\s\d+\sduplicates"):tuple(["duplicates"]),
            re.compile("(\d+)\s\+\s\d+\smapped\s\((\S+)%\s\+\s\S+%\)"):tuple(["nr_mapped","percent_mapped"]),
            re.compile("(\d+)\s\+\s\d+\spaired in sequencing"):tuple(["seq_paired"]),
            re.compile("(\d+)\s\+\s\d+\sproperly paired \((\S+)%\s\+\s\S+%\)"):tuple(["nr_proper_pairs","percent_proper_pairs"]), 
            re.compile("(\d+)\s\+\s\d+\swith itself and mate mapped"):tuple(["nr_proper_with_mate"]),
            re.compile("(\d+)\s\+\s\d+\ssingletons\s\((\S+)%\s\+\s\S+%\)"):tuple(["singletons", "percent_singletons"]),
            re.compile("(\d+)\s\+\s\d+\sread1"):tuple(["nr_read1"]),
            re.compile("(\d+)\s\+\s\d+\sread2"):tuple(["nr_read2"]),
            re.compile("(\d+)\s\+\s\d+\swith mate mapped to a different chr \(mapQ.+\)\Z"):tuple(["chr_mismatch_q5"]),
            re.compile("(\d+)\s\+\s\d+\swith mate mapped to a different chr\Z"):tuple(["chr_mismatch"])
        }
